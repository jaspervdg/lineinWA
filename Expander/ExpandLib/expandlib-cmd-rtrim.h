...
/** \file expandlib-cmd-rtrim.h
	\brief Headerfile for the 'rtrim' command
*/

#ifndef EXPANDLIB_CMD_RTRIM_H_INCLUDED
#define EXPANDLIB_CMD_RTRIM_H_INCLUDED

#include "expandlib-cmd.h"

namespace expander { // *** Start of namespace

	/// The class implementing the 'rtrim' command
	/** Syntax:<br>
		\#rtrim(stringtotrim,charstotrim)	Trims a string
	*/
	...
	template<
		...
		>
	class expcmd_rtrim : public expandcmd<StringType> {
	protected:
	public:
		/// The constructor
		...
		*/
		expcmd_rtrim(expandable<StringType> * const eobj) : expandcmd<StringType>(eobj, _T("rtrim")) {};
		...
		virtual void operator()(StringType &dest, const FParamVectorType &params, const FVarMapType &vars, ExpandResultsType &results) const
		{
			if ( params.size() == 2 ) {
				...
				dest.append(d.begin(), d.begin()+d.find_last_not_of(params[1])+1);
			} else {
				throw expand_error(_T("expcmd_rtrim"),_T("Parameter error"));
			}
		}
	};
...
#endif //EXPANDLIB_CMD_RTRIM_H_INCLUDED
