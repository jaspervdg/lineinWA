...
/** \file expandlib-cmd-ftrunc.h
	\brief Headerfile for the 'ftrunc' command
*/

#ifndef EXPANDLIB_CMD_FTRUNC_H_INCLUDED
#define EXPANDLIB_CMD_FTRUNC_H_INCLUDED
...
namespace expander { // *** Start of namespace
	...
	/// The class implementing the 'ftrunc' command
	/** Syntax:<br>
		\#ftrunc(stringtotruncate,numcharstotruncateto[,truncendstring])	Truncates a string (from the front)
	*/
	...
	template<
		...
		>
	class expcmd_ftrunc : public expandcmd<StringType> {
	protected:
	public:
		/// The constructor
		...
		*/
		expcmd_ftrunc(expandable<StringType> * const eobj) : expandcmd<StringType>(eobj, _T("ftrunc")) {};
				...
				try {
					...
					if ( d.size() > num ) {
						if ( params.size() == 3 ) {
							...
							if ( e.size() >= num ) e=_T("");
							dest.append(d.begin(), d.begin()+num-e.size());
							dest+=e;
						} else {
							dest.append(d.begin(), d.begin()+num);
						}
					} else {
						dest+=d;
					}
				} catch(boost::bad_lexical_cast) {
					throw expand_error(_T("expcmd_ftrunc"),_T("Illegal size"),params[1].c_str());
				}
			} else {
				throw expand_error(_T("expcmd_ftrunc"),_T("Parameter error"));
			}
		}
	};
...
#endif //EXPANDLIB_CMD_FTRUNC_H_INCLUDED
